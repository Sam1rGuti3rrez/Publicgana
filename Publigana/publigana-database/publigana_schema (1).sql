...
-- Datos de perfil público (foto, biografía corta)
ALTER TABLE usuario ADD COLUMN foto_url VARCHAR(255);
...
-- Redes sociales personales del usuario (para su Perfil, ej. @instagram propio)
CREATE TABLE red_social_usuario (
    id            UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    usuario_id    UUID NOT NULL REFERENCES usuario(id) ON DELETE CASCADE,
    red_social_id UUID NOT NULL REFERENCES red_social(id),
    handle        VARCHAR(150) NOT NULL,   -- @usuario o URL del perfil
    created_at    TIMESTAMP WITHOUT TIME ZONE DEFAULT NOW(),
    UNIQUE (usuario_id, red_social_id)
);

CREATE INDEX idx_red_social_usuario ON red_social_usuario(usuario_id);
